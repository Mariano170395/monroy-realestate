...
const Oportunidades = () => {
  ...
  return (
    <div>
      ...
      <div>
        <h1 className="TituloRedes">Oportunidades</h1>
      ...
      <div className="WrapperCatalogo">
      {
        images.filter(image=>{
          ...
          image.estatus === 'Oportunidades' && image.colonia.includes(busqueda) && (image.banos ===  parseInt(baños)|| baños === '') && (image.habitaciones ===  parseInt(habitaciones)|| habitaciones === '')  && (precioMin === '' || image.precio >= parseInt(precioMin)) && (image.precio <= parseInt(precioMax) || precioMax === '') ? (

          <div  onClick={()=>history.push(`/oportunidades/${image._id}`)} key={image._id}>
